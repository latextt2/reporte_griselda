...
En [paper número de polos]  se estudia de manera más detallada los efectos de variar el número de polos en un RMR, se concluye que utilizar 6 polos minimiza el diámetro exterior para una capacidad dada.
...
Estos modos de operación puede implementarse usando el circuto de la figura [...], donde el arreglo de comparadores y flip flops permite controlar la corriente con una precisión del $n\%$.
...
Entre las diferentes composiciones de imanes permanentes destaca el Neodimio-Hierro-Boro comúnmente conocidos como imanes de neodimio. Los valores más altos $H_c$ y  $B_r$ son alcanzados por los imanes de grado $N52$, con valores de $B_r = 1400mT$ y $H_c=796kA/m$ [véase anexo B]. 
...
Donde: \\
    ...
    $\mu$ = Coeficiente de rozamiento la rosca y le tornillo \\
...
Finalmente se ejecuta otro script que varía la distancia entre el objetivo y el sensor para obtener la curva característica mostrada en la figura [curva caracterisitica...]
...
Podemos definir este nuevo maco de referencia como la transformación $T_O^B$ como se muestra en la ecuación:
...
Sustituyendo los términos descritos en la sección \ref{sec:modelo-dinamico} en la ecuación \ref{eq:derivada-inercia-coriolis} se obtiene el sistema de ecuaciones [sistema de ecuaciones]. 

[instertar ecuacion espacio de estados del sistema] 

Estas ecuaciones se introducen dentro del modelo de \textbf{Simulink} mostrado en la figura [modelo simulink] para realizar la simulación.   

[figura Simulink]

\subsection{Linealización}

Podemos observar que este sistema de ecuaciones es no lineal, así que tenemos que linealizarlo para convertirlo en la forma canónica de espacio de estados, y aplicar las técnicas de control descritas en la sección \ref{sec:control} mediante las formulas \ref{eq:control:jacobiano}. 

Lo que resulta en el sistema descrito por las matrices:

[insertar matrices sin evaluar]  

\subsection{Controlador}

Con el fin de estabilizar el sistema descrito en la ecuación [sistema linealizado] utilizamos el esquema de control retroalimentado presentado en la sección [control retroalimentado]. 

Se selecciona un conjunto de polos para el nuevo sistema y se utiliza el script de \textbf{Matlab}[anexo] para calcular las ganancias del controlador $K$. Este esquema de control es implementado en el modelo de \textbf{Simulink} mostrado en la figura [...]. 

[insertar esquema de control retroalimentado]

El esquema de control mostrado anteriormente es capaz de estabilizar el sistema pero no puede alcanzar un error de estado estable igual a $0$, por lo que se agrega un integrador y un vector de ganancias con el fin de producir un nuevo esquema como el mostrado en la sección [...]. Este nuevo vector de ganancias es calculado por el script [...] y su modelo de Simulink es el que se muestra en la figura [...].

[insertar modelo simulink]

\subsection{Observador}

Para realizar control en espacio de estados es necesario retroalimentar el estado completo del sistema. En el caso del sistema descrito en [...] no se puede medir de manera directa todo los parámetros del vector de estados por lo que es necesario un observador de estados como es descrito en la sección [ref observador de estados]. 

Las ganancias del observador de estados $K_o$ son calculadas mediante el script [...]. El modelo de \textbf{Simulink} correspondiente a este observador es mostrado en la figura [...].

[figura modelo Simulink]

\subsection{Control calendarizado}

Con el fin de compensar los efectos de las no linealidades provocadas por la precisión del eje utilizamos el método de control calendarizado descrito en la sección [...], en donde linealizamos en diferentes valores de $\Omega$ para obtener el conjunto de matrices de transición $A$, $B$, $C$ y el conjunto de ganancias $K$ y $K_o$ para el observador de estados. Estos cálculos son realizados por el script [...] y el conjunto de matrices es interpolado durante la simulación del modelo de \textbf{Simulink} mostrado en la figura [...]. 
...
Como se indica en la sección [...], para emular un sistema controlador-observador es necesario transformar los polos que definen la respuesta del sistema del dominio de Laplace al dominio de $Z$. Esto se realiza mediante la transformación de Tustin expresada en la ecuación [ecuacion de tustin].

Para el calculo de las ganancias del controlador discreto, es necesario discretizar el sistema continuo y, con el nuevo conjunto de polos discretos, aplicar las técnicas de ubicación de polos descritas en la sección [...]. 

El sistema queda implementado en el modelo de \textbf{Simulink} mostrado en la figura [...]. Cabe resaltar que lo que era un integrador en el sistema continuo ahora es un acumulador en lugar de un retardo unitario ($Z^{-1}$).  

[insertar modelo de simulink]

\subsection{Observador de Kalman}

Si el ruido en los sensores alcanza niveles excesivos la estabilidad del controlador podría verse comprometida, por motivos de seguridad s necesario contar con un sistema más robusto en cuanto a inmunidad al ruido. Esto puede alcanzarse mediante el filtro de Kalman. 
 
El filtro de Kalman es un observador en tiempo discreto que permite mejorar la calidad de nuestras mediciones gracias a que incorpora información sobre la dinámica del sistema y los niveles de ruido (covarianza) presente en las mediciones de los sensores. En el caso del rodamiento magnético se añade la ventaja de que integra la información redundante de los tres sensores de posición radial. 

A partir del sistema discretizado se aplican las formulas del filtro de Kalman descritas en la sección [...]. Cabe resaltar que las ganancias del filtro de Kalman son calculadas en linea y de manera iterativa, a diferencia de las ganancias del observador clásico. 

El filtro queda implementado en el modelo de \textbf{Simulink} mostrado en la figura [...]. 
