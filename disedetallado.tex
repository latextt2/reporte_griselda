...
El procedimiento realizado a continuación consiste en obtener un modelo dinámico del eje, linealizar la representación del sistema, especificar la respuesta deseada, calcular las ganancias del controlador-observador en modo continuo y discretizar el conjunto controlador-observador. Además mediante el control calendarizado se compensarán los efectos de controlar un sistema no lineal usando una aproximación linealizada. Finalmente se discuten los efectos que puede tener el ruido de los sensores y su solución mediante la implementación del observador de Kalman. 
...
Podemos observar que este sistema de ecuaciones es no lineal, así que tenemos que linealizarlo para convertirlo en la forma canónica de espacio de estados, y aplicar las técnicas de control descritas en la sección \ref{sec:control} mediante las fórmulas \ref{eq:control:jacobiano}. 
...
Para el cálculo de las ganancias del controlador discreto, es necesario discretizar el sistema continuo y, con el nuevo conjunto de polos discretos, aplicar las técnicas de ubicación de polos descritas en la sección [...]. 
...
Si el ruido en los sensores alcanza niveles excesivos la estabilidad del controlador podría verse comprometida, por motivos de seguridad es necesario contar con un sistema más robusto en cuanto a inmunidad al ruido. Esto puede alcanzarse mediante el filtro de Kalman. 
...
A partir del sistema discretizado se aplican las fórmulas del filtro de Kalman descritas en la sección [...]. Cabe resaltar que las ganancias del filtro de Kalman son calculadas en línea y de manera iterativa, a diferencia de las ganancias del observador clásico. 
