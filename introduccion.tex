...
Gran parte de la maquinaria industrial posee elementos de tipo rotativo, los cuales requieren el uso de dispositivos denominados cojinetes, también conocidos como rodamientos o baleros. Estos suelen situarse entre dos componentes de la máquina con un eje de rotación común, facilitando el movimiento de giro de un componente con respecto al otro y reduciendo la fricción de los diferentes elementos móviles. Los rodamientos son a su vez, un punto de apoyo para dichos ejes o árboles. 

Además de ser capaces de disminuir la fricción, de manera general los rodamientos deben cumplir con algunas características particulares como son: disminuir las vibraciones, operar de manera silenciosa y poseer una prolongada vida útil. Sin embargo, los distintos sectores industriales suelen establecer sus propios requisitos específicos con base en determinados entornos operativos o aplicaciones particulares, entre los que podemos mencionar: resistir grandes cambios de temperatura, humedad, suciedad, operar en medios agresivos como soluciones ácidas o alcalinas, soportar altas velocidades e incluso ser aptos para el contacto con alimentos  \cite{cite_1}.
...
Los rodamientos magnéticos cuentan, a su vez, con una clasificación que viene en función de dos principales factores: de acuerdo a la naturaleza de su fuerza, que puede ser de Reluctancia o de Fuerza de Lorentz, o más comúnmente de acuerdo a la fuente generadora del campo magnético (Pasivos, Activos e Híbridos)\cite{cite_4}. 
...
Pese a las múltiples ventajas que presentan, los rodamientos magnéticos no están exentos de algunos problemas que pueden dificultar su desarrollo e implementación en los sistemas industriales, entre los que podemos destacar el alto consumo energético que generan.\par
Ante este problema se hicieron varias investigaciones cuyas soluciones pueden clasificarse en dos tipos: el diseño de algoritmos para minimizar las corrientes en los electroimanes y el uso de imanes permanentes para proveer un campo magnético base para la levitación.\par
La investigación realizada en \cite{cite_5} se trata de una validación experimental en donde primero se diseña y construye un prototipo de pruebas que emula el comportamiento de un rodamiento magnético activo de un grado de libertad, y segundo se realizaron pruebas para evaluar el desempeño del controlador inteligente en comparación con un controlador estándar de polarización fija.\par
Este equipo consiste en un eje rígido capaz de balancearse libremente sobre un punto de apoyo colocado debajo de su centro de masa. La posición angular de este eje es controlada mediante electroimanes fijados a los extremos de este. A pesar su simplicidad este equipo incorpora todas las no linealidades típicas de un rodamiento magnético activo, y la dinámica del sistema es ideal para evaluar el desempeño de los controladores.\par
En \cite{cite_6} una estrategia de control de corriente de polarización variable es presentada con el fin de minimizar la energía consumida sin alterar el desempeño dinámico del sistema. El modelado parte de un RMA de 2 polos sobre el cual se propone un controlador lineal y un observador de estados para la estimación de estados. Una vez obtenido el modelo del sistema se calcula de manera analítica la corriente óptima para la estabilización del sistema y obtener el controlador.\par
...
Han reducido dramáticamente el tamaño de los rodamientos magnéticos axiales y radiales mediante el uso de nuevas e innovadoras técnicas de diseño y manufactura. Todas estas modificaciones han reducido en general la complejidad del sistema \cite{cite_9}.
...
Todos estos beneficios implican una reducción significativa de los costos inherentes al mantenimiento, relacionados principalmente al reemplazo de piezas debido al desgaste, además de la eliminación de sistemas adicionales de refrigeración y lubricación y a una necesidad de mantenimiento periódico menor \cite{cite_10}. 

A pesar de las múltiples ventajas que ofrece este tipo de sistemas, no se ha logrado ampliar su uso dentro de la industria debido principalmente a que presentan otro tipo de desventajas que no se encuentran en los cojinetes y rodamientos, como su accesibilidad limitada, un precio más elevado, la necesidad de contar con un sistema de control y la posibilidad de fallas en el sistema eléctrico, además de que en el caso de los rodamientos magnéticos activos el consumo eléctrico suele ser elevado.

Es por eso que en este sentido se optó por el desarrollo de rodamientos magnéticos de funcionamiento híbrido, ya que al utilizar un campo base otorgado por los imanes permanentes, el consumo eléctrico de los electroimanes es menor. Este proyecto tendrá como finalidad desarrollar un sistema que pueda brindar una alternativa a los rodamientos magnéticos tradicionales en proyectos relacionados con el desarrollo de maquinaria y equipo industrial.

\newpage

\section*{Descripción de los Capítulos}
\subsubsection{Capítulo 1: Marco Teórico}

Este capítulo tiene como finalidad describir los conocimientos fundamentales aplicados al proyecto durante el desarrollo del mismo. Entre los más importantes están la comprehensión de los fenómenos de electricidad y magnetismo, el control de sistemas lineales y discretos, los filtros de Kalman, entre otros. Asimismo, refuerza y repasa los conocimientos sobre electrónica y conceptos de diseño mecatrónico (PDS y análisis funcionales) para permitir una mejor vinculación entre los elementos matemáticos de control y el fenómeno de levitación magnética.

\subsubsection{Capítulo 2: Dise\~no Conceptual}

El objetivo del capítulo es describir la importancia del desarrollo teórico de este prototipo, así como el impacto industrial que tiene. Marca las acotaciones de las especificaciones sobre las que se diseñó el rodamiento, y establece a grosso modo la morfología del prototipo. Define también las áreas funcionales sobre las que se trabajó durante la gestación del proyecto, y muestra un análisis morfológico para determinar de forma general un diseño apropiado de que presentación final tendría el prototipo.

\subsubsection{Capítulo 3: Dise\~no Detallado}

A lo largo de este capítulo se describe el desarrollo de cada área funcional a detalle basándose en los procedimientos descritos en el capítulo 1 y retoma los bocetos del diseño conceptual en el capítulo 2. Esto con el fin de que para el final de este capítulo, se obtenga una simulación de un prototipo que cumpla los requerimientos planteados previamente. Abarca distintas pruebas aplicando cálculos teorizados sobre simulaciones para que a partir de ellas, se compare el comportamiento de cada uno de los módulos y se determinen los elementos con mejores resultados para implementarlos en la simulación del prototipo final en conjunto.

\subsubsection{Capítulo 4: Validaci\'on y An\'alisis de Resultados}

Este capítulo tiene la finalidad de integrar todas la áreas funcionales y ver su comportamiento a través de simulaciones específicas, donde se muestra el funcionamiento en conjunto de los elementos previamente seleccionados en el capítulo 3, así como la respuesta ante ciertas perturbaciones que podrían suscitarse en el área de aplicación donde se sitúe dicho prototipo. De igual forma, se muestran los alcances de este prototipo bajo ciertos parámetros de funcionamiento.
