...
Donde $\omega$ es una  constante de perturbacion. El  itegrador $z(k + 1) = z(k) + r -y(k)$ al agregarlo al modelo anterior obtenemos:
...
Designando $ F = [F_1 F_2]$ tal que $|\lambda(A + BF)| < 1$ \\
Lema: $(A,B)$ es controlable(respectivamente estabilizables) si solo si:
...
Es una salida retroalimentada asumiendo que $(A,C)$ es detectable designando a un observador que estime $x$ por $\hat{x}$ y tome:
...
\todo[inline]{diagrama}
\todo[inline]{formulas}
La discritización (o digitalizacion) de controladores continuos es basicamente emplear metodos de discretizacion para el procesamiento digital de señales. Existen varion metodos para la discretizacion controladores continuos, en este caso particular se utiliza el metodo de la transformada bilinear (trapezoidal, o tustin).

Este método, desarrollado por el profesor Arnold Tustin, es una técnica de mapeo de polos y ceros de una función de tiempo continuo para un sistema de desplazamiento invariante discreto, describe $H(z)$ de un sistema LTI, descrito por un $H (s)$ (pasa del plano  $s$ al plano $z$). La transformada bilineal se define como :
%\arrowvert_
\begin{equation}
	H(z) = H(s) \bracevert_{s = \frac{2}{t}(\frac{1 - z^{-1}}{1 + z^{-1}})} 
\end{equation}   

Toma la funcion $H(s)$ y todas sus variables acompañados por $s$ y sustituye los terminos equivalentes en $z$. Lo que esto implica que si el sistema es controlable y es estable al hacer la tranformacion garantiza la controlabilidad y la estabilidad en $z$.


\begin{figure}[htb]
\centering
	\includegraphics[width=0.75\textwidth]{images/Capitulo_1/mapeo_s_z}
	\caption{\textit{Tranformacion de bilineal.}}
\end{figure}
