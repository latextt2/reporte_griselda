...
\todo[inline]{Se define el momento angular, la Ley de Newton para cuerpo regido, y los problemas para relizar los calculos}
...
\begin{equation}
	\sum_i{M_i} = \dot{H} = [I] \dot{\omega} + \omega \times H
\end{equation}
...
donde se tiene que
\todo[inline]{formula ;v}
aplicandolo a la formula ?? tenemos que
\todo[inline]{formula ;v}
como los ejes ahora se mueven con el objecto la matriz de inercia se mantiene constante, y por tanto sus derivada son cero, si consideramos que la velocidad angular del cuerpo y la velocidad angular del marco mobil $\Omega$ es la misma entonces podemos expandir la ecuacion ?? en la forma

las ecuaciones ?? se conocen como las ecuaciones de euler, y nos proveen de un medio para calcular el movimiento de un cuerpo rigido en tres dimensions, sin embargo estas ecuciones presentan problemas para encotrar sus soluciones ya que estas poseen productos de $\omega$ y esto representa no linealidades. El otro problema consiste en definir la forma en la que se represetaran la rotacion del marco mobil.

\todo[inline]{formula :v}
\begin{equation}
	M_x = \dot{H_x} - H_y\omega_z + H_z\omega_y
\end{equation}
\begin{equation}
	M_y = \dot{H_y} - H_z\omega_x + H_x\omega_z
\end{equation}
\begin{equation}
	M_z = \dot{H_z} - H_x\omega_y + H_y\omega_x
\end{equation}
...
\todo[inline]{Se introducen los angulos de euler, y porque estos solucion el problema de calcular el momento angular}
% primera parte
\begin{equation}
...
\begin{equation}
	{H^B}_1 (\phi,\theta,\psi)= {H_2}^B (\phi) {H_1}^2 (\theta) {H_l}^1 (\psi) 	
\end{equation}

\begin{equation}
	{H^B}_1 (\phi,\theta,\psi)= \begin{pmatrix}
	1 & 0 & 0\\
	0 & cos(\phi) & sin(\phi)\\
	0 & -sin(\phi) & cos(\phi)
	\end{pmatrix} \begin{pmatrix}
...
\begin{equation}
	...
	0\\
	0
	\end{pmatrix} + {H^B}_2 \begin{pmatrix}
	0\\
	\dot{\theta}\\
	0
	\end{pmatrix} + {H^B}_2{H^2}_1 \begin{pmatrix}
	0\\
	0\\
	\dot{\psi}
	\end{pmatrix}
...
\begin{equation}
	...
	\dot{\theta}\\
	\dot{\psi}
	\end{pmatrix} = {L^B}_l \Theta
\end{equation}
$trasformada inversa [(.)^{-1} \neq (.)^T]$
\begin{equation}
	\begin{pmatrix}
	\dot{\phi}\\
	\dot{theta}\\
	...
	q\\
	r
	\end{pmatrix} = {L^l}_B W_B
\end{equation}


\subsubsection{Las ecuaciones de Euler}
\todo[inline]{Se presenta la version a usar de las ecucaciones de Euler}
\subsubsection{Precesion}
\todo[inline]{Se explican los efectos de nutacion y precesion como resultado de la ecuaciones de euler}

\pagebreak
\section{Control}
\todo[inline]{Se explica el objectivo del contro}
\subsection{Linearizacion}
\todo[inline]{Se presentan las formulas y el proceso para linearizar un sistema no lineal, en variables de estado}
