...
Es por esto que, para el desarrollo de este proyecto, fue necesario aplicar una serie de procesos con base en las investigaciones y el desarrollo que han tendido tanto a nivel industrial como académico. Se realizaron consultas de diversos artículos y sitios web de fabricantes y desarrolladores que han logrado comercializar estos productos para la industria, y se efectuó una investigación documental sobre trabajos de varios grados e instituciones con el fin de marcar una referencia durante el desarrollo de este proyecto. Con base en la información adquirida, fue considerado lo siguiente:
...
Donde  $\mu_0$ es la permeabilidad magnética del vacío, una constante universal con valor $4\pi\times10^{-7} \frac{H}{m}$.
...
\begin{equation}
	\phi = \int_{s}^{} \overrightarrow{B} \cdot d \overrightarrow{s}
\end{equation}
...
Para hablar sobre el fenómeno de magnetización comenzaremos por definir los dominios magnéticos. Los dominios magnéticos son agrupaciones de dipolos magnéticos. Cuando el material no es naturalmente magnetizado los átomos están apuntando en distintas direcciones sin cohesión, pero en presencia de un campo creciente los dipolos se van agrupando y apuntando en la dirección. El material se magnetiza. 
...
Sin embargo, para otros medios (particularmente materiales ferromagnéticos), $B$ no es una función lineal de $H$ ni posee un mismo valor de $B$ para uno de $H$.
...
En este caso ninguna corriente externa está presente para un campo magnetizante por $B = \mu_0 M$. Podemos ver que la magnetización $M$ y el campo magnetizante $H$ contribuyen a la densidad de flujo en maneras similares. 
...
El material se  determina a partir de su histéresis, coercitividad y remanencia para estas aplicaciones. Es importante resaltar que estas propiedades magnéticas dependen fuertemente de los tratamientos a los que fue sometido el material. En años recientes se han descubierto algunos materiales para imanes permanentes basados en neodimio-hierro-boro, donde su coercitividad puede ser tan alta como $1.12x10^6 K \frac{A}{m}$, comparada con $0.72x 10^6 K \frac{A}{m}$ para el samario-cobalto.
...
\begin{center}
\centering
	\includegraphics[width=.6\textwidth]{images/Capitulo_1/Elementos_de_un_circuito_magnetico}
...
La ecuación \ref{eq:electroiman:fuerza-ideal} puede encontrarse en la literatura \cite{cite_s4_2017} \cite{cite_s6_2017}   \cite{cite_s7_2017}. Esta ecuación se considera un caso ideal, ya que son despreciados los efectos del flujo de fuga, flujo marginal y la reluctancia del núcleo. En el caso de un entrehierro pequeño la reluctancia del núcleo podría volverse significativa causando errores en el cálculo de la fuerza. Esta situación podría detectarse mediante una simulación de elemento finito. 
...
Si comparamos los resultados de estas ecuaciones con la simulación de elemento finito podemos observar un porcentaje de error en la fuerza ejercida. Esto representa una gran desviación comparada con los resultados del electroimán radial.
...
Entonces para el correcto modelado del electroimán necesitamos incluir los efectos del flujo marginal. En el trabajo realizado por J. Mühlethaler, J.W. Kolar y A. Ecklebe \cite{cite_s8_2017_paper} se presenta un nuevo método para modelar la reluctancia de un entrehierro.    
...
En (J. Mühlethaler, J.W. Kolar, A. Ecklebe)\cite{cite_s8_2017_paper} se presenta un método para modelar la reluctancia tomando en cuenta el flujo marginal, el cual considera el flujo en tres dimensiones, es fácil de manejar y cuenta con un alto grado de exactitud. 
Este método surge a partir de aplicar la transformación Schwarz-Christoffel al sistema de la figura \ref{fig:Geometria-basica-gap} con el fin de calcular su reluctancia por unidad de longitud (reluctancia planar). Esta reluctancia queda determinada por: 
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{images/Capitulo_1/Geometria-basica-gap}
    \caption{Geometría básica para el cálculo del entrehierro \cite{cite_s8_2017_paper}.}
    \label{fig:Geometria-basica-gap}
\end{figure}

La figura \ref{fig:Geometria-basica-gap} se toma como base para calcular estructuras de entrehierro más complejas. La figura \ref{fig:Geometrias-avanzadas-gap} muestra cómo se pueden alcanzar diferentes geometrías a partir del modelo básico, además de establecer los valores de $\omega$ y $l$. El valor $h$ representa la distancia desde el borde del entrehierro a la siguiente esquina del n\'ucleo.

\begin{figure}[H]
    \centering
    \includegraphics[scale=.7]{images/Capitulo_1/Geometrias-avanzadas-gap}
    \caption{Tipos de geometría planar del entrehierro \cite{cite_s8_2017_paper}.}
    \label{fig:Geometrias-avanzadas-gap}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{images/Capitulo_1/Gap-3D}
    \caption{Geometría 3D del entrehierro \cite{cite_s8_2017_paper}.}
\end{figure}
...
\subsection{Cálculo de conductores} \label{sec:calc-conductores}
...
\hline 
AWG & Di\'ametro & Área\;($mm^2$) & Resistencia (ohms/km) & Corriente Máxima & Frecuencia Máxima\\ 
\hline 
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.4\textwidth}
        ...
        \caption{Circuito magnético de imán permanente.}
    \end{subfigure}
    ~ %add desired spacing between images, e. g. ~, \quad, \qquad, \hfill etc. 
      %(or a blank line to force the subfigure onto a new line)
    \begin{subfigure}[b]{0.3\textwidth}
        \includegraphics[width=5cm, height=5cm]{images/Capitulo_1/Geometria_de_iman_y_ferrita_guia}
        \caption{Geometría de imán y ferrita guía.}
        \label{fig:empaq-hex}
    \end{subfigure}
    \caption{Geometría de un conjunto imán permanente-núcleo ferromagnético.}\label{fig:paq-bobina}
\end{figure}
...
\begin{multline} \label{eq:def_lti}
...
Un sistema discreto se caracteriza porque su variable independiente sólo puede tomar determinados valores y es descrito mediante una ecuación en diferencias con coeficientes constantes: 
...
Para cada matriz $A$ de orden $m$ se tiene que en $det(A - \lambda I)$ es un polinomio de grado $m$ que se llama polinomio característico de $A$ y se denota por $pA(\lambda)$. La ecuación $det(A - \lambda I) = 0$ se llama ecuación característica de $A$, donde se denota $I$ como la matriz identidad. Las relaciones que ligan las entradas de una matriz con los coeficientes del polinomio característico son muy complicadas. No obstante, ciertos coeficientes de dicho polinomio son conocidos.
...
Al igual que en el caso continuo, un sistema discreto es estable si se presenta un cambio pequeño en las entradas, un sistema estable presentará modificaciones pequeñas en su respuesta. La estabilidad se puede determinar a partir de la localización de los polos de lazo cerrado en el plano z. El sistema es estable si los polos de las raíces de la ecuación característica quedan localizadas dentro del círculo unitario en el plano Z. Por el contrario, si un par de polos conjugados complejos están fuera el círculo unitario o polos múltiples el sistema es inestable, pero si al menos un polo  está ubicado en $z=1$ o $z = -1$, el sistema es marginalmente estable.

\newpage

\section{Dinámica}
\subsection{Movimiento traslacional}
...
\end{equation} 
Este conjunto de ecuaciones se les conoce como ecuaciones de Euler, y nos proveen de un medio para calcular el movimiento de un cuerpo rígido en tres dimensiones. Sin embargo, estas ecuaciones presentan problemas para encontrar sus soluciones ya que poseen productos de $\omega$ y esto representa no linealidades. El otro problema consiste en definir la forma en la que se represetará la rotación del marco móvil.
...
Nótese que la matriz ${Q_B^l}$ se indetermina para valores de $\theta = \pm\pi/2$. Para todos los juegos de ángulos de Euler existen puntos donde la matriz inversa se indetermina, por eso es importante la correcta selección del juego de ángulos.
...
Se dice que un sistema es controlable si se puede transferir desde cualquier estado inicial $x(t_0)$ a cualquier otro estado mediante un vector de control sin restricciones en un intervalo de tiempo finito.\\
...
\begin{equation}
	...
	\end{pmatrix}
\end{equation}
El sistema descrito por las ecuaciones es completamente observable sí y solo sí la matriz ${M_o}^T$ es de rango n ó $M_o$ tiene n vectores fila linealmente independientes.

\subsection{Control retroalimentado} \label{sec:control-retroalimentado}
...
Consideremos el esquema de control de la figura \ref{fig:control:control-retroalimentado}, en este esquema de control conocido como retrolimentación de estado completo, se usa como entrada el producto de vector de estados $x$ y la matriz de ganancias $K$ de manera que:
...
Existen varios métodos para calcular la matriz $K$ entre los que destacan el método de Kalman y el método de Ackerman. Para el caso de sistemas con una sola entrada existen soluciones únicas para esta matriz, sin embargo, para sistemas con múltiples entradas existen infinita cantidad de soluciones. Por tanto, el método de Kalman y Ackerman no funcionan para estos últimos.

Debido a las infinitas soluciones, se necesitan restricciones adicionales para obtener soluciones concretas. Un método que resulta robusto es el desarrollado por J. Kautsky, N.K. Nichols y P.Van Dooren \cite{cite_s9_2017_Kautsky}, donde se minimiza la sensitivdad del sistema a las perturbaciones. Este método es el que implementa el comando 'place' de Matlab.

\subsubsection{Observador de estados} \label{sec:observador-estados}

Un observador de estado estima las variables de estado basándose en las mediciones de las variables de salida y de control. El concepto de observabilidad juega un papel importante.
...
\subsection{Control integral continuo} \label{sec:control-integral}
...
\subsection{Control calendarizado} \label{sec:control-calendarizado}
...
\subsection{Discretización de controladores} \label{sec:discretizacion-controladores}
...
\begin{equation}
	...
	\label{eq:ecuacion-Tustin}
\end{equation}   

Toma la función $H(s)$ y todas sus variables acompañadas por $s$ y sustituye los términos equivalentes en $z$. Lo que esto implica que si el sistema es controlable y es estable, al hacer la tranformación se garantiza la controlabilidad y la estabilidad en $z$.

\begin{figure}[H]
\centering
	\includegraphics[width=0.5\textwidth]{images/Capitulo_1/mapeo_s_z}
	\caption{\textit{Tranformación bilineal.}}
\end{figure}
...
\begin{figure}[H]
\centering
	\includegraphics[width=.85\textwidth]{images/Capitulo_1/Ciclo_Vida}
...
\begin{figure}[H]
\centering
	\includegraphics[width=.85\textwidth]{images/Capitulo_1/Disen_mecatronico}
...
\begin{figure}[htb]
\centering
	\includegraphics[scale=.4]{images/Capitulo_1/Funciones1}
...
\begin{figure}[htb]
\centering
	\includegraphics[scale=.4]{images/Capitulo_1/Funciones2}
...
\begin{figure}[htb]
\centering
	\includegraphics[width=.85\textwidth]{images/Capitulo_1/Funciones3}
	\caption{\textit{Delimitación de la frontera.}}
\end{figure}

%En la figura \ref{fig:area-funcional-ejemplo} se muestra un ejemplo de diagrama de funciones para un sistema más complejo, donde se aprecia cómo se agrupan los diferentes subsistemas de acuerdo a las funciones que realizan y la forma en que se relacionan. 
...
%	\label{fig:area-funcional-ejemplo}
%\end{figure}
